...
EOT
  ...
  validation {
    ...
    error_message = "[from validate.WorkspaceName: invalid when len(value) == 0]"
  }
  validation {
    condition = alltrue([
      for k, v in var.databricks_workspaces : (
    ...
    error_message = "[from validate.WorkspaceName: invalid when len(value) < 3]"
  }
  validation {
    condition = alltrue([
      for k, v in var.databricks_workspaces : (
    ...
    error_message = "[from validate.WorkspaceName: invalid when len(value) > 64]"
  }
  validation {
    condition = alltrue([
      for k, v in var.databricks_workspaces : (
